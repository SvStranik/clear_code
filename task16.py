...
# "Избыточные коментарии" - функция простая и коментарий не требуется
# "Не используйте комментарии там, где можно использовать функцию или переменную" - возможно
# лучше в название добавить кодировку - "PrintingCostsInASCII".
...
# "Избыточные коментарии" - функция простая и коментарий не требуется
# "Не используйте комментарии там, где можно использовать функцию или переменную" - возможно
# лучше изменить название "DisplaysNumOfVotes".
...
# "Избыточные коментарии" - данная функция совершенно не требует коментарий.
...
# "Избыточные коментарии" - данная функция совершенно не требует коментарий, также
# для того чтобы не описывать, что является условием форматирования и для наглядности
# константу "REFERENCE_SYMBOL" можно перенести в параметры функции.

def LineAnalysis(CHARACTER_STRING, REFERENCE_SYMBOL = '*'):
    is_Found = False
...
# "Избыточные коментарии" - функция является простой, в комментарии не требуется,
# возможно под функцией указал бы об обязательном четном числе.
...
# "Избыточные коментарии" - в названии класса уже понятно, что работает со строками.
...
def codeEncripted(source_string):
    ...
    # "Избыточные коментарии" - данный коментарий не нужен сдесь.
...
# По всем курсам которые проходил коменттарии ни где не делал, так как изначально 
# выучил, что коментарий - это плохо и в моих заданиях оно по настоящему не требовался 

# Из данного курса на собственных задачах уяснил, что при правильном названии переменных
# код читается хорошо и понятно без коментариев.
