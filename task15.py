def codeEncripted(source_string):
    # "Усиление" - метод "source_string.replace(' ','')" - очищаем текст от пробелов, для последующего форматирования
...
# "Усиление" - Важно, используется таблица раскладки символов ASCII

# "Предупреждения о последствиях" - В случае неучтенного символа в "TEXT_SYMBOL_PRINT" 
...
# "Представление намерений" - Функция распознает закономерность в тексте изображений точек и звездочек. При 
# правильной закономерности текст считаестя корректным. 

# "Прояснение" - Константа "REFERENCE_SYMBOL" обозначает условие проверки текста изображения
...
# "Информативные коментарии" - Функция, подсчитывает количество голосов "CANDIDATES_VOTES", кондидатов "NUM_CANDIDATES"
...
# "Информативные коментарии" - Функция вращает елементы матрици по часовой стрелке относительно ее центра.  

# "Усиление" - Получает на вход массив строк "matrix_data" (по ссылке!)  и вращает ее же!

# "Прояснение" - Минимальное значение "NUMBER_OF_COLUMNS, NUM_SPINS" обязательно четное!
...
# "Предупреждения о последкствиях" Наследник текущего класса, который работает только со строковым типом.

class OrderedStringList(OrderedList):
    def __init__(self, el_sort_ascending):
        super(OrderedStringList, self).__init__(el_sort_ascending)

    # "Усиление" - сравниваем строки предварительно очищенные от пробелов с двух сторон
...
# "Информативные коментарии"Функция вычисляет заранее двери которые будут открыты по завершению работы мастера ключей.
...
# "Информативные коментарии" - Функция определяет максимальную скидку при покупке товаров. Получаем на вход "DISCOUNT_STEP"
# кол-во товара для приобретения аукционного товара и список цен на те товары которые необходимо 
# преобрести "PRICE_PURCHASE_ARRAY"
